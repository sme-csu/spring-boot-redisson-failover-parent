...
public class ClusterConnectionManager extends MasterSlaveConnectionManager {
    ...
    private final static Logger log = LoggerFactory.getLogger(ClusterConnectionManager.class);
    ...
    public ClusterConnectionManager(ClusterServersConfig cfg, Config config, UUID id) {
        ...
        for (String address : cfg.getNodeAddresses()) {
            ...
            try {
                ...
                log.info("sync client address:{}",connection.getRedisClient().getAddr());
                StringBuilder nodesValue = new StringBuilder();
                for (ClusterNodeInfo clusterNodeInfo : nodes) {
                    nodesValue.append(clusterNodeInfo.getNodeInfo()).append("\n");
                    log.info("sync nodes info:{}",clusterNodeInfo);
                    log.info("sync address info:{}",clusterNodeInfo.getAddress());
                }
    ...
    private void scheduleClusterChangeCheck(ClusterServersConfig cfg, Iterator<RedisURI> iterator) {
        log.info("ClusterServersConfig:{}",cfg);
        log.info("node address:{}",cfg.getNodeAddresses());
    ...
    private void updateClusterState(ClusterServersConfig cfg, RedisConnection connection,
        ...
        future.onComplete((nodes, e) -> {
            ...
            for (ClusterNodeInfo clusterNodeInfo : nodes) {
                nodesValue.append(clusterNodeInfo.getNodeInfo()).append("\n");
                log.info("async node info:{}",clusterNodeInfo);
                log.info("async address info:{}",clusterNodeInfo.getAddress());
            }
            log.info("cluster nodes state got from {}:\n{}", connection.getRedisClient().getAddr(), nodesValue);

            log.info("nodes:{}",nodes);
            List<ClusterNodeInfo> replacedClusterNodes =  replaceByReachableNode(nodes,connection.getRedisClient().getAddr().toString());
            log.info("replacedClusterNodes: ",replacedClusterNodes);
            Collection<ClusterPartition> newPartitions;

            if(CollectionUtils.isEmpty(replacedClusterNodes)){
                newPartitions = parsePartitions(nodes);
            }else {
                newPartitions = parsePartitions(replacedClusterNodes);
            }
    ...
    private static List<ClusterNodeInfo> replaceByReachableNode(List<ClusterNodeInfo> nodes,String clientAddress){
        if(CollectionUtils.isEmpty(nodes)){
            return null;
        }
        String clientIp = getClientIpAddress(clientAddress);
        log.info("clientIp:{}", clientIp);
        ClusterNodeInfo firstNode =  nodes.get(0);
        log.info("firstNode:{}", firstNode);
        if(!isReachableHost(firstNode.getAddress().getHost(),firstNode.getAddress().getPort())){
            log.warn("the host is unreachable,host ip is {}",firstNode.getAddress().getHost());
            List<ClusterNodeInfo> replacedNodes = new ArrayList<>();
            nodes.forEach( clusterInfo ->{
                String hostIp = clusterInfo.getAddress().getHost();
                log.info("hostIp:{}", hostIp);
                String nodeInfo =  clusterInfo.getNodeInfo().replace(hostIp,clientIp);
                log.info("nodeInfo:{}",nodeInfo);
                ClusterNodeInfo replaceNode = new ClusterNodeInfo(nodeInfo);
                replacedNodes.add(replaceNode);
                log.info("address: " + clusterInfo.getAddress().toString());
                log.info("replaced address: " + clusterInfo.getAddress().toString().replace(hostIp,clientIp));
                replaceNode.setAddress(clusterInfo.getAddress().toString().replace(hostIp,clientIp));
                replaceNode.setSlaveOf(clusterInfo.getSlaveOf());
                log.info("slave of : " + clusterInfo.getSlaveOf());
                replaceNode.setNodeId(clusterInfo.getNodeId());
                log.info("nodeId : " + clusterInfo.getNodeId());
                replaceNode.setSlotRanges(clusterInfo.getSlotRanges());
                log.info("slotRanges : " + clusterInfo.getSlotRanges());
                replaceNode.setFlags(clusterInfo.getFlags());
                log.info("flag : " + clusterInfo.getFlags());
            });
    ...
    private static String getClientIpAddress(String clientAddress){
        Assert.isTrue(!StringUtils.isEmpty(clientAddress),"The host address is empty.");
        String[] addressInfo =  clientAddress.replace("/","").split(":");
        Assert.isTrue(addressInfo.length ==2,"The host address format is wrong.");
        return addressInfo[0];
    }

    /*private static String getHostIpAddress(String hostAddress){
        System.out.println("**hostAddress:" + hostAddress);
        Assert.isTrue(!StringUtils.isEmpty(hostAddress),"The host address is empty.");
        String[] hostAddresses = hostAddress.substring(8).split(":");
        Assert.isTrue(hostAddresses.length >=2,"The host address format is wrong.");
        return hostAddresses[hostAddresses.length-2];
    }*/

    private static boolean isReachableHost(String hostAddress,int port) {
       return isReachable(hostAddress,port,100);
    }
